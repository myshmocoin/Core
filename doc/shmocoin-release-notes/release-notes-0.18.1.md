ShmoCoin Core version 0.18.1 is now available from:

  <https://download.shmocoin.org/shmocoin-0.18.1/>
...
`/Applications/ShmoCoin-Qt` (on Mac) or `shmocoind`/`shmocoin-qt` (on
Linux).
...
ShmoCoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use ShmoCoin Core on unsupported systems.
...
10.10. Additionally, ShmoCoin Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:24263:24263` (this is an extra `:24263` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@shmocoin.org) the ShmoCoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 SHMO),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in SHMO per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/shmocoin-conf.md)
  about the `shmocoin.conf` file describes how to use it to configure
  ShmoCoin Core.

- A new document introduces ShmoCoin Core's BIP174 [Partially-Signed
  ShmoCoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent ShmoCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  ShmoCoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when ShmoCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of ShmoCoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart shmocoind with the `-deprecatedrpc=generate`
...
- A new `shmocoin-wallet` tool is now distributed alongside ShmoCoin
...
This section describes planned changes to ShmoCoin Core that may affect
other ShmoCoin software and services.

- Since version 0.16.0, ShmoCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with ShmoCoin Core 0.20 (expected about
  a year after 0.18), ShmoCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the ShmoCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in ShmoCoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any ShmoCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, ShmoCoin Core
...
- A sub-project of ShmoCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with ShmoCoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to ShmoCoin Core's own implementation, although entropy
  gathered by ShmoCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves ShmoCoin Core a
...
- On macOS, ShmoCoin Core now opts out of application CPU throttling
...
- #24263 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
