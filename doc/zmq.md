...
The ShmoCoin Core daemon can be configured to act as a trusted "border
router", implementing the shmocoin wire protocol and relay, making
...
The ZeroMQ feature in ShmoCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building shmocoind:
...
For instance:
    ...
    $ shmocoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/shmocoind.tx.raw \
...
These options can also be provided in shmocoin.conf.
...
From the perspective of shmocoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into shmocoind directly. Furthermore, no information is
...
using. ShmoCoind appends an up-counting sequence number to each
