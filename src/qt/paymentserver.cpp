...
const QString BITCOIN_IPC_PREFIX("shmocoin:");
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start shmocoin: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("shmocoin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'shmocoin://' is not a valid URI. Use 'shmocoin:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid ShmoCoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
